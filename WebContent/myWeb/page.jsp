...
<%@ taglib prefix="fn" uri="http://java.sun.com/jsp/jstl/functions"%> 
        	<div class="pagination previous-next" style='text-align:center;'>
				...
				<input type='text' class='form-control' style='display:inline;width:60px;' value=''/><a class='btn btn-info next' <c:if test="${p.totalPage==0||p.pageNo==p.totalPage }">disabled</c:if> href='javascript:void(0);' onclick='location.href="${pageContext.request.contextPath }${param.url }?pageNo="+(this.previousSibling.value)+"${param.param }"'>跳转</a>
			</div>